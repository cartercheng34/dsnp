...
#define toSizeT(t)      (SIZE_T+SIZE_T*(t/SIZE_T))  // TODO
//
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      _activeBlock -> reset();
      for(size_t i=0;i<_recycleList.size();i++){
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      if(m == n)
         ...
         bool find = true;
         MemRecycleList<T>* tmp = _recycleList[n];
         while(true){            
            if(find == false)
               break;
            if(tmp->getArrSize() == n){
   ...
   T* getMem(size_t t) {
      ...
      if(ret != 0){
         ...
         if(_activeBlock->getMem(t,ret) == false){
            size_t rn_active = _activeBlock->getRemainSize();
            size_t rn = getArraySize(rn_active);
            _activeBlock->getMem(rn_active,ret);
            #ifdef MEM_DEBUG
            cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
            #endif // MEM_DEBUG
            getMemRecycleList(rn)->pushFront(ret); //new a block
            _activeBlock->reset();  //reset 
