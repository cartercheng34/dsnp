...
CmdParser::parseCmd(string& option)
{
   ...
   CmdExec* tmp = getCmd(token);
   if(tmp == 0){
      cerr<<"Illegal command!! (" << token << ")";
      return NULL;
   }
   else if(str == token)
      return tmp;

   option = str.substr(counter + 1);
   return tmp;
...
CmdParser::listCmd(const string& str)
{
   ...
   string substr = str.substr(0,int(_readBufPtr - _readBuf));
   string tok;
   size_t tmp = myStrGetTok(substr , tok);
   vector <string> cmdstored;        
   while(tok.size()>0){
      cmdstored.push_back(tok);
      tmp = myStrGetTok(substr , tok , tmp);
   }//ok
   ...
   else if(cmdstored.size() == 1){
      string ss;
      if(str[str.size()-1] == ' '){
         for(CmdMap::const_iterator iter = _cmdMap.begin();iter != _cmdMap.end();iter++){
            ss = iter->first + iter ->second->getOptCmd();
            if(myStrNCmp(ss , cmdstored[0] , iter->first.size()) == 0){
               cout<<endl;
               getCmd(ss)-> usage(cout);
            }
         }
      }     
      else{
         ...
         for(CmdMap::const_iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
            ss = iter->first + iter->second->getOptCmd();
            if(ss.size() >= cmdstored[0].size() && myStrNCmp(ss, cmdstored[0], cmdstored[0].size()) == 0)
               more.push_back(iter->first + iter->second->getOptCmd());
         }
         
         if(more.size() == 0)   
            mybeep();
         if(more.size() == 1){
            for(size_t i = cmdstored[0].size(); i < more[0].size(); i++){
               insertChar(more[0][i]);
            } 
            insertChar(' ');
         }
            ...
            cout << endl;
            for(size_t i = 0, count = 0; i < more.size(); i++){
               cout << setw(12) << left << more[i];
               ++ count;
               if(count == 5){
                  count = 0;
                  cout << endl;
