   ...
   if(doRow == true){
     ...
     for(size_t i=0 ; i<dbtbl.nCols() ; i++){
        if(i>=tokens.size()-1 || tokens[i+1] == "-") 
            add.push_back(INT_MAX);
        else{
            int tmp;
            myStr2Int(tokens[i+1] , tmp);
            add.push_back(tmp);
        }
     }
     dbtbl.addRow(add);
...
{  
   ...
   else if(checkRowIdx(tokens[0], x)){      
      if (size == 1) 
         return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
      if (!myStr2Int(tokens[1] , y))   
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[1]);
      if (checkColIdx(tokens[1] , y)){         
         if(dbtbl.getData(x,y) == INT_MAX)  
            cout<<"null\n";
         else cout<<dbtbl.getData(x,y)<<endl;
      }
      else return CMD_EXEC_ERROR;
   }
